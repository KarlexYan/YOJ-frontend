<template>
  <div id="addQuestionView">
    <div
      ...
      "
    >
      <template v-if="route.path.startsWith('/examination_question/update')">
        修改套题题目信息
      </template>
      <template v-else>创建套题题目</template>
    </div>
    <a-form
      :model="form"
      label-align="left"
...
import { ExaminationQuestionControllerService } from "../../../backapi";
...
const loadData = async () => {
  if (updatePage) {
    ...
    const res =
      await ExaminationQuestionControllerService.getExaminationQuestionByIdUsingGet(
        id as any
      );
    if (res.code === 0) {
...
const doSubmit = async () => {
  ...
  if (updatePage) {
    const res =
      await ExaminationQuestionControllerService.updateExaminationQuestionUsingPost(
        {
          ...form.value,
    ...
    const res =
      await ExaminationQuestionControllerService.addExaminationQuestionUsingPost(
        {
          ...form.value,
          examinationId: route.query.id as unknown as number,
        }
      );
    if (res.code === 0) {
