<template>
  <div id="examinationSubmitView">
    <a-form :model="searchParams" layout="inline" style="margin-left: 300px">
      <a-form-item field="examinationId" label="题号" tooltip="请输入套题ID">
        <a-input
          v-model="searchParams.examinationId"
          placeholder="请输入查询题号"
        />
      </a-form-item>
      <a-form-item
    ...
    >
      ...
      <template #examinationId="{ record }">
        <a-link @click="toExaminationPage(record)"
...
// 搜索请求
const searchParams = ref<ExaminationSubmitQueryRequest>({
...
const loadData = async () => {
  const res =
    await ExaminationControllerService.listExaminationSubmitByPageUsingPost({
      ...searchParams.value,
...
const toExaminationPage = (examinationId: ExaminationSubmitQueryRequest) => {
  router.push({
    path: `/examination/view/${examinationId.examinationId}`,
  });
};
